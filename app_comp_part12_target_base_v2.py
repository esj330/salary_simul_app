...
from comp_core_target_base_v2 import simulate_all
...
def generate_pdf(summary_p1, team_p1_df, exec_p1_df, summary_p2):
    ...
    for _, row in team_p1_df.iterrows():
        ...
        draw_text(f"1인 기본연봉(억, 기본급+인센티브): {row['1인 연봉(억)']}")
...
def main():
    ...
    # "팀장·팀원 최종 연봉 합계(억)" = (팀장 1명 최종연봉 × 팀장 인원수) + (팀원 1명 최종연봉 × 팀원 인원수)
    team_total_final = leader_final_salary_per * n_leaders + member_final_salary_per * n_members
    ...
    # ===== Part1 요약표 =====
    st.subheader("2) Part 1 결과 – 예상 매출 기준 연봉 & 법인 손익(세금·경비 제외 기준)")
    ...
    summary_p1 = pd.DataFrame(
        [
            ...
            [
                ...
                (
                    "(팀장 1명 최종연봉 × 팀장 인원수) + (팀원 1명 최종연봉 × 팀원 인원수) "
                    f"= ({leader_final_salary_per:.4f} × {n_leaders}) + ({member_final_salary_per:.4f} × {n_members})"
                ),
            ],
            [
                "결과",
